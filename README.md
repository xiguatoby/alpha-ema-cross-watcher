...
轻量级 Alpha 代币 EMA 金叉监控器：基于 OKX DEX K线，检测 15m EMA144/EMA576 金叉并推送 Telegram



## 5) `watcher.py`（重构后的去隐私主程序）
```python
import os
import time
import hmac
...
def detect_strict_cross(df: pd.DataFrame, fast_col: str, slow_col: str, lookback: int = 3) -> List[int]:
    """
    返回发生严格金叉的索引（负索引：-3,-2,-1）
    严格金叉定义：prev_fast <= prev_slow and curr_fast > curr_slow
    """
    signals: List[int] = []
    for i in range(-lookback, 0):  # e.g., -3,-2,-1
        prev = df.iloc[i - 1]
        curr = df.iloc[i]
